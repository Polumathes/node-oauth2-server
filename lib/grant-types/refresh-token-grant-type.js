...
RefreshTokenGrantType.prototype.readRequestBody = async (request) => {
...
RefreshTokenGrantType.prototype.getRefreshToken = async function(request, client) {
  ...
  if (!is.vschar(body.refresh_token)) {
    throw new InvalidRequestError('Invalid parameter: `refresh_token`');
  }

  return promisify(this.model.getRefreshToken, 1).call(this.model, body.refresh_token)
    .then(function(token) {
      if (!token) {
