...
AuthorizationCodeGrantType.prototype.readRequestBody = async (request) => {
...
AuthorizationCodeGrantType.prototype.getRequestParams  = (url) => {
...
AuthorizationCodeGrantType.prototype.getAuthorizationCode = async function(request, client) {
  ...
  if (!is.vschar(body.code)) {
    throw new InvalidRequestError('Invalid parameter: `code`');
  }
  return promisify(this.model.getAuthorizationCode, 1).call(this.model, body.code)
    .then(function(code) {
      if (!code) {
 ...
 */
 ...
 AuthorizationCodeGrantType.prototype.validateRedirectUri = async function(request, code) {
   if (!code.redirectUri) {
     return;
   }
