...
AuthenticateHandler.prototype.handle = async function(request, response) {
  ...
  try{
      let req_token = await this.getTokenFromRequest(request)
      const token = await this.getAccessToken(req_token)
      await this.validateAccessToken(token)
      if ('scope' in this && !!this.scope) {
...
AuthenticateHandler.prototype.updateResponse = function(response, accessToken) {
    ...
    out.user = accessToken.user
    out.scope = accessToken.scope
    response = new Response(JSON.stringify(out),response)
    ...
    return { token: out, accepted_scope: this.scope }
};
